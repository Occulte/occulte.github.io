...
(function(){
  ...
  function parseAuthorsDetailed(authorsStr) {
    ...
    return parts.map(p => {
      ...
      return { display, star }; // no highlight flag
    });
  }
  ...
  async function render(){
    ...
    try { const prefix = (location.pathname.match(/^.*\//) || [""])[0];
      history.replaceState({}, "", prefix + encodeURIComponent(key)); } catch(_){}
  ...
  <div class="detail-authors author-grid">
    ${authorsArr.map((a, i) => {
      ...
      return `<span class="author-item">${name}${star}${nums}</span>`;
    }).join("")}
  </div>`;
