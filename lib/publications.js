...
function mapBibEntryToPub(e) {
  ...
  const cleanAuthors = authorsRaw
    ? authorsRaw.replace(/[*†]/g, "") // 去掉 * 和 †
    : "";
...
function pubTemplate(p, idx) {
  ...
  const venue = (p.venue || p.year) ? `<span>${sanitizeHTML([p.venue, p.year].filter(Boolean).join(", "))}</span>` : "";
