...
// - The BibTeX shown in UI contains ONLY title/author/journal/year.
...
// Template identical to original, but uses minimal bibtex built above.
function pubTemplate(p, idx){
  ...
  const authors = p.authors ? `<div class="pub-meta">${sanitizeHTML(p.authors)}</div>` : "";
  const links = [
